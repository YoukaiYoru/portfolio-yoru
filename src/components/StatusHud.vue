...
const daysTooltip = computed(() => `${ageStats.value.remainingDays} dias para cumplir ${ageStats.value.nextAge}`);
...
onMounted(() => {
  ...
  const show = (target: Element) => {
    ...
    tip.style.left = `${targetRect.left - hostRect.left + targetRect.width / 2}px`;
    tip.style.top = `${targetRect.top - hostRect.top - 10}px`;
  ...
  <div ref="root" class="status-hud" :style="{ width: props.width }" :data-birthday="props.birthday">
    ...
    <svg viewBox="0 0 540 640" xmlns="http://www.w3.org/2000/svg">
      ...
      <g>
        ...
        <text class="t-cap abbr js-dias" x="454" y="284" tabindex="0" :data-tip="daysTooltip">DIAS<title>Dias para tu cumpleaños</title></text>
        <text x="340" y="320" text-anchor="middle" font-size="28" font-weight="500">
          <tspan class="js-day-cur" fill="var(--ink)">{{ ageStats.elapsedDays }}</tspan><tspan class="t-dim js-day-max">/{{ ageStats.totalDays }}</tspan>
        </text>

        <text class="t-stat abbr" x="246" y="396" text-anchor="end" tabindex="0" data-tip="Energia">EN<title>Energia</title></text>
        <text class="t-big" x="296" y="400" font-size="50">
          <tspan>{{ props.en }}</tspan><tspan class="t-dim" font-size="32"> /{{ props.enMax }}</tspan>
        </text>

        <text class="t-stat abbr" x="194" y="454" text-anchor="end" tabindex="0" data-tip="Foco · concentracion">FOC<title>Foco · concentracion</title></text>
        <text class="t-big" x="244" y="458" font-size="46">
          <tspan>{{ props.foc }}</tspan><tspan class="t-dim" font-size="28"> /{{ props.focMax }}</tspan>
        </text>

        <text class="t-stat abbr" x="144" y="512" text-anchor="end" tabindex="0" data-tip="Skills · habilidades acumuladas">SKL<title>Skills · habilidades acumuladas</title></text>
